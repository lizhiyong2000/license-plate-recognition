...
def tiltCorrection(vertices, rect):
    ...
    if rect[2] > -45:
        ...
        # # 校正后的四个顶点坐标
        point_set_1 = np.float32([[240, 0],[0, 0],[0, 80],[240, 80]])
    # 畸变情况2
    elif rect[2] < -45:
        ...
        # # 校正后的四个顶点坐标
        point_set_1 = np.float32([[0, 0],[0, 80],[240, 80],[240, 0]])
    ...
    if not len(contours):  # contours1长度为0说明未检
        ...
        for ii, cont in enumerate(contours):
            ...
            if np.mean(img_cut_mask) >= 75 and w > 15 and h > 15:
                ...
                #
                vertices, rect = findVertices(approx)
                #
            ...
            #     # print(box)
                ...
                p0, p1, _ = tiltCorrection(vertices, rect)
                ...
                cv2.drawContours(img_src_copy, p0, -1, (255, 0, 0), 2)
